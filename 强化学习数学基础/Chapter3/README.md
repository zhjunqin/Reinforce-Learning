...
在 $\gamma = 0$ 的极端情况下，对应的最优策略如图下图所示。在这种情况下，Agent 无法到达目标区域。这是因为每个状态的最优策略都是极其短视的，仅仅选择具有最大即时奖励的动作，而不是最大的总回报。
...
$$return_{\pi_1} = 1 + \gamma 1 + \gamma^21+... = \frac{1}{1-\gamma} = 10 $$

$$return_{\pi_2} = 0 + \gamma 0 + \gamma^21+... = \frac{\gamma^2}{1-\gamma} = 8.1 $$
